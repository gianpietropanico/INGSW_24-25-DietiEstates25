...
class AdminApiImpl @Inject constructor (private val httpClient: HttpClient) : AdminApi {
    ...
    override suspend fun getAllSuppAdmins(request: UserInfoRequest): ListResponse<List<User>> {

        val response = httpClient.get("http://10.0.2.2:8080/admin/users") {
    ...
    override suspend fun addSuppAdmin(request: AdminRequest): ApiResult<String> {
        return try{
            val response = httpClient.post("http://10.0.2.2:8080/admin/users") {
                contentType(ContentType.Application.Json)
                accept(ContentType.Application.Json)
                setBody(request)
            }
